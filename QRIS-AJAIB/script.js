...
window.buatPembayaran = async function () {
  ...
  const res = await fetch(`https://www.alfaofficial.cloud/api/orkut/createpayment?apikey=${apikey}&amount=${jumlahDeposit}&codeqr=${qrisCode}`);
...
async function cekStatusPembayaran() {
  ...
  try {
    const res = await fetch(`https://www.alfaofficial.cloud/api/orkut/cekstatus?apikey=${apikey}&merchant=${merchantId}&keyorkut=${keyorkut}`);
    const json = await res.json();
